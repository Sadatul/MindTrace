...
fun ScreenRegister(
    ...
    // Auto-trigger sign-in if we just signed out (for account switching)
    LaunchedEffect(Unit) {
        // Check if this is likely an account switch scenario:
        // - No current Firebase user
        // - No cached user info
        // - No current dialogs showing
        val firebaseUser = com.google.firebase.auth.ktx.auth.currentUser
        val cachedUserInfo = com.example.frontend.api.SelfUserInfoCache.getUserInfo()
        
        if (firebaseUser == null && cachedUserInfo == null && !uiState.showRegisterPrompt && !uiState.showRoleDialog) {
            // Small delay to ensure UI is ready
            kotlinx.coroutines.delay(500)
            
            Log.d(TAG, "Auto-triggering Google Sign-In for account switch...")
